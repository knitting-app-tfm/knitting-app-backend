...
def firebase_register(body: FirebaseRegisterRequest, db: Session = Depends(get_db)) -> UserResponse:
    try:
        user = user_service.register_with_credentials(db, body.email, body.password, body.username)
...
@router.post(
    ...
    description=(
        "Verifies a Firebase token and creates the user in the database. "
    ),
)
